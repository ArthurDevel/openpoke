...
function computeSmoothingCoefficient(
  timeMs: number,
  sampleRate: number,
  chunkLength: number
): number {
...
export class RMSNormalizer {
  private readonly targetRmsLinear: number;
  private readonly peakLimitLinear: number;
  ...
  process(audio: Int16Array): Int16Array {
    ...
    const desiredGain =
      rms > this.silenceThresholdLinear ? this.targetRmsLinear / rms : this.currentGain;
